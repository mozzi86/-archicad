...
- [x] **CAP-03:** AUFGELÖST 2026-07-14 (Markierungsrahmen-Workflow, ChangeSelectionOfElements; s. bulk-operations.md). Ursprünglicher Stand: partial. `elements_get_selected_elements` ist da, aber in der Live-Session war die Selektion leer — eigentliches Selektion-vs-Modal-Verhalten konnte nicht stringent re-getestet werden. Bestätigt aus Phase-3-Session: „Einstellungen für die Raum-Auswahl" → Selektion weg bei Abbrechen.
- [x] **CAP-04:** AUFGELÖST 2026-07-14 (Raumnummer = numberStr, create-only, direkter Tapir-Abruf; s. bulk-operations.md). Ursprünglicher Stand: partial. `properties_get_property_values_of_elements` ist verifiziert (CAP-01 Read-Pfad). Built-in „Raumnummer"-Property exists (Zone hat 278 Built-in Properties), aber der konkrete Property-Name für Raumnummer wurde in dieser Session nicht final isoliert. Pragmatisch: bei Schedule-Pipeline lieber via `numberStr` aus Zone-Detail-Fields lesen (wenn `get_details_of_elements`-Bug behoben ist), oder via Schedule-Export.
