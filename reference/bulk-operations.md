...
**Fortschreibung 2026-09-01 — derselbe Effekt bei Preisen, und die Property ist diesmal komplett tot.** <!-- 2026-09-01 --> Im selben Projekt zwei Jahresviertel später: „Möbel Vertragsart" ist inzwischen sauber gepflegt (534 Architektur / 445 RV Standard / 175 RV Innovativ / 124 Schreiner / 44 Funktional), aber die alte „Preisklasse" trägt weiterhin den Default — **479 Architekturmöbel stehen dort auf „RV Standard"**. Eine nicht gepflegte Property wird nicht von selbst besser; sie bleibt ein Default-Feld und gehört stillgelegt oder per Expression abgeleitet, nicht „abgeglichen".

Neu und teurer ist der Preis-Fall: Die Property „Preis Netto" steht bei **allen 1.322 Möbeln auf 0,000**. Die echten Einkaufspreise liegen im GDL-Parameter „Einkauf netto" und erscheinen **nur im Schedule-Export**, nicht über `GetPropertyValuesOfElements`. Wer Kosten per API aus dem Modell zieht, bekommt eine saubere, erfolgreiche Antwort mit lauter Nullen. **Regel:** Vor jeder Kosten-/Preisauswertung von Library-Objekten entweder über den [Schedule-Export](schedule-pipeline.md) gehen oder an bekannt unterschiedlichen Objekten gegenprüfen, dass die Property überhaupt trägt. Passt zur Skill-Regel „Ein `success` ist kein Beleg".

### TeamWork: Bulk-Set scheitert pro-Element mit Code 6001

**Zuerst Ebene und Hotlink prüfen, dann erst Reservierung.** <!-- 2026-09-01 --> Live-Tag mit 103 erfolgreichen Property-Sets: **kein** 6001 des Tages war eine Reservierungsfrage. Ursachen in dieser Reihenfolge abklopfen:
1. **Ebene ausgeblendet.** Elemente auf ausgeblendeten Ebenen sind nicht änderbar; Archicad meldet das im Teamwork als 6001 — auch nach `ReserveElements success` und auch nach manueller UI-Reservierung (22 Elemente auf `50 Möblierung Küche OG 1` / `Küchen EG`). Prüfen über `API.GetLayerAttributes` → `isHidden`; die Element-Property „Ebene geschützt" zeigte dabei `False` und war irreführend. Rezept: Tapir `CreateLayers {layerDataArray:[{name, isHidden:false}], overwriteExisting:true}` → `ReserveElements` → Set → Rücklese → Ebene wieder `isHidden:true` → `ReleaseElements`. Danach 17/17 und 5/5.
2. **Hotlink.** BuiltIn-Property „Hotlink Master-ID" (`F98F297A-AFA7-4B0D-824B-975C78E02995`) gefüllt (z. B. „Hochbau") → nur in der Quelldatei änderbar; auch `SetClassificationsOfElements` scheitert mit 6001.
3. Erst dann fremde Reservierung — die sieht nur der Nutzer im UI-Dialog.

Nebenbefund: Tapir `DeleteElements` meldete `success:true`, das Element auf der ausgeblendeten Ebene war danach noch da — Rücklese per `GetElementsByType` Pflicht.
